***Memo
*no covariates are used except for lagged outcomes
*Only baseline SC estimation adopt "nested allopt", in order to avoid estimation time and optimization errors


foreach name in demean_oldage_inkind demean_soexp_health ///
				/*demean_lfp_30to34_fe  demean_lfp_35to39_fe demean_lfp_40to44_fe  demean_lfp_45to49_fe*/ /// 
				/*demean_lfp_50to54_fe  demean_lfp_55to59_fe demean_d_lfp_50to54_fe  demean_d_lfp_55to59_fe*/ { ///demean = demeaned outcome, demeaned_d = demeaned outcome of DDD-type outcome
...
*synth
 synth $outcome $pred_lag,  xperiod($first_year(1)$pre_t_year)  mspeperiod($first_year(1)$pre_t_year) ///
